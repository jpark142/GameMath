...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	CameraObject& mainCam = g.GetMainCamera();
	
	TransformComponent& playerTransform = goPlayer.GetTransform();

	Vector3 inputVec = Vector3(input.GetAxis(InputAxis::XAxis), input.GetAxis(InputAxis::YAxis), input.GetAxis(InputAxis::ZAxis)).GetNormalize();
	playerTransform.AddPosition(inputVec * moveSpeed * InDeltaSeconds);
	playerTransform.AddPitchRotation(-input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);

	mainCam.SetLookAtRotation(playerTransform.GetPosition());
